...
export function ParentSettings({ settings, onChange, onReset, onBack }: Props) {
  ...
  return (
    <section className={`settings-page page-shell ${showInstallGuide ? "install-guide-open" : ""}`}>
      ...
      <img className="settings-bear" src={assetPath("assets/optimized/characters/bear.jpg")} alt="" loading="eager" decoding="async" />

      <footer className="settings-footer">
        <p className="footer-english">© 2026 Little Ears StoryBox · All rights reserved</p>
        <p className="footer-contact">X: @hann7712&nbsp;&nbsp;·&nbsp;&nbsp;Wechat: Canaan77</p>
        <p className="footer-chinese">素材仅供本应用使用，转载商用请先联系作者 · 内容不断更新中</p>
      </footer>
    </section>
  );
}
